...
| 事件               | 回调参数                       | 触发时机                          |
| ------------------ | ------------------------------ | --------------------------------- |
| `loadstart`        | `total: number`                | 开始下载模型资源                  |
| `loadprogress`     | `loaded, total, file`          | 每个文件下载完成                  |
| `loaded`           | —                              | 所有资源下载完成，首帧渲染开始    |
| `tap`              | `areaName: string`             | 点击命中 Hit Area                 |
| `motionstart`      | `group, index, duration, file` | 动作开始播放                      |
| `motionend`        | `group, index, file`           | 动作播放结束                      |
| `expressionchange` | `id: string`                   | 调用 `setExpression()` 时立即触发 |
| `destroy`          | —                              | 模型销毁完成                      |
