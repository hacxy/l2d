...
class L2DExpressionMotion extends AMotion {
  ...
  static loadJson(buf) {
    ...
    ret.setFadeIn(Number.parseInt(json.fade_in) > 0 ? Number.parseInt(json.fade_in) : 1000);
    ret.setFadeOut(Number.parseInt(json.fade_out) > 0 ? Number.parseInt(json.fade_out) : 1000);
    if (json.params == null) {
      return ret;
    }
    ...
    for (let i = 0; i < paramNum; i++) {
      ...
      let value = Number.parseFloat(param.val);
      ...
      if (calcTypeInt == L2DExpressionMotion.TYPE_ADD) {
        ...
        let defaultValue = param.def == null ? 1 : Number.parseFloat(param.def);
        if (defaultValue == 0)
          defaultValue = 1;
        value = value / defaultValue;
      }
...
class L2DPhysics {
  ...
  static load(buf) {
    ...
    for (let i = 0; i < paramNum; i++) {
      ...
      const length = Number.parseFloat(setup.length);
      const resist = Number.parseFloat(setup.regist);
      const mass = Number.parseFloat(setup.mass);
...
export { L2DBaseModel, L2DEyeBlink, L2DMatrix44, L2DTargetPoint, L2DViewMatrix, Live2DFramework };
