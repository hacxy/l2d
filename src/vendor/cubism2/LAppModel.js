...
class LAppModel extends L2DBaseModel {
  ...
  update() {
    ...
    if (this.mainMotionManager.isFinished()) {
      if (this._motionIsPlaying) {
        this._motionIsPlaying = false;
  ...
  setRandomExpression() {
    ...
    const no = Number.parseInt(Math.random() * tmp.length);
    this.setExpression(tmp[no]);
  }

  startRandomMotion(name, priority) {
    const max = this.modelSetting.getMotionNum(name);
    const no = Number.parseInt(Math.random() * max);
